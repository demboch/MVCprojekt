...
namespace MVCprojekt.Controllers
{
    ...
    public class AccountController : Controller
    {
       // private ApplicationDbContext db = new ApplicationDbContext();
        ...
        //{
        //    var dostepne = (from b in db.Users
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", "Nieprawidłowe hasło lub nazwa użytkownika.");
                }
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            using (var context = new ApplicationDbContext())
                if (ModelState.IsValid)
                {
                    var informacjeKlient = new InformacjeKlient() { KlientId = model.KlientId, Imie = model.Imie, Nazwisko = model.Nazwisko, Wiek = model.Wiek, Pesel = model.Pesel, Numer_Telefonu = model.Numer_Telefonu };
                    var user = new ApplicationUser { UserName = model.Email, Email = model.Email, informacjeKlient = informacjeKlient };
                    var result = await UserManager.CreateAsync(user, model.Password);
                    //var roleStore = new RoleStore<IdentityRole>(context);
                    //var roleManager = new RoleManager<IdentityRole>(roleStore);
                    //var userStore = new UserStore<ApplicationUser>(context);
                    //var userManager = new UserManager<ApplicationUser>(userStore);
                    ////user.Id = (string)informacjeKlient.KlientId;
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.RemoveLoginSuccess ? "Twoje konto zewnętrzne zostało usunięte."
